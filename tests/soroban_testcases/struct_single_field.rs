...
fn single_uint256_field() {
    // FIXME: small values aren't encoded/decoded correctly
    let runtime = build_solidity(
        r#"
        contract test {
            struct S { uint256 v; }
...
fn single_int128_field() {
    ...
    // FIXME: bug in encoding/decoding i128
    // let res = runtime.invoke_contract(
    //     addr,
    //     "combine",
...
fn single_int256_field() {
    // FIXME: small values are not encoded/decoded correctly
    let runtime = build_solidity(
        r#"
        contract test {
            struct S { int256 v; }
